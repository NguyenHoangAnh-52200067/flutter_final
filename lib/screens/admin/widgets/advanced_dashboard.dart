...
class _AdvancedDashboardState extends State<AdvancedDashboard> with SingleTickerProviderStateMixin {
  final currencyFormat = NumberFormat.currency(locale: 'vi_VN', symbol: '₫');
  TimeFrame selectedTimeFrame = TimeFrame.yearly;
  DateTime? startDate;
  DateTime? endDate;
    ...
    try {
      ...
      for (var doc in ordersSnapshot.docs) {
        ...
        for (var item in items) {
          final productId = item['product']['id'] as String;
          final quantity = (item['quantity'] as num).toInt();
          final costPrice = (item['product']['costPrice'] as num?)?.toDouble() ?? 0.0;
