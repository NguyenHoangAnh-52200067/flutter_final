...
class _AdminUserManagementScreenState extends State<AdminUserManagementScreen> with SingleTickerProviderStateMixin {
  ...
  void _filterUsers(String query) {
    setState(() {
      _filteredUsers = _users.where((user) {
        ...
        final emailLower = user.email.toLowerCase();
        final searchLower = query.toLowerCase();
        return fullNameLower.contains(searchLower) || emailLower.contains(searchLower);
      }).toList();
      _currentPage = 1;
      _updateTotalPages();
    });
                                    ...
                                    return FadeTransition(
                                      ...
                                      child: ScaleTransition(
                                        ...
                                        child: Card(
                                          ...
                                          child: ListTile(
                                            leading: CircleAvatar(
                                              backgroundImage: user.linkImage != null && user.linkImage!.isNotEmpty
                                                  ? NetworkImage(user.linkImage!)
                                                  : NetworkImage(
                                                      'https://th.bing.com/th/id/OIP.52T8HHBWh6b0dwrG6tSpVQHaFe?rs=1&pid=ImgDetMain',
                                                    ),
            ...
            child: Column(
              ...
              children: [
                ListTile(
                  ...
                  title: const Text('Xem chi tiết', style: TextStyle(color: Colors.black87)),
                  onTap: () {
                    Navigator.pop(context);
                    _showUserDetails(user);
                  },
                ...
                ListTile(
                  ...
                  title: const Text('Cập nhật thông tin', style: TextStyle(color: Colors.black87)),
                  onTap: () {
                    Navigator.pop(context);
                    UpdateUserProfileDialog.show(context, user, (updatedUser) {
                      setState(() {
                ...
                ListTile(
                  ...
                  title: FutureBuilder<bool>(
                    ...
                    builder: (context, snapshot) {
                      return Text(
                        snapshot.data == true ? 'Mở khóa tài khoản' : 'Khóa tài khoản',
                        style: TextStyle(
                          color: snapshot.data == true ? Colors.green : Colors.red,
                        ),
                      );
              ...
              child: Column(
                ...
                children: [
                    ...
                    children: [
                      CircleAvatar(
                        ...
                        backgroundImage: user.linkImage != null && user.linkImage!.isNotEmpty
                            ? NetworkImage(user.linkImage!)
                            : null,
                        child: user.linkImage == null || user.linkImage!.isEmpty
                  ...
                  _buildDetailItem(
                    Icons.location_on,
                    'Địa chỉ',
                    (_addressRepository.getAddressesByUserId(user.id!)).toString(),
                    Colors.green,
                  ),
                  const SizedBox(height: 15),
                  _buildDetailItem(
  ...
  Future<void> disableUser(String uid) async {
    try {
      final isCurrentlyBanned = await _userRepository.isUserBanned(uid);
      final success = isCurrentlyBanned
          ? await _userRepository.unbanUser(uid)
          : await _userRepository.banUser(uid);
