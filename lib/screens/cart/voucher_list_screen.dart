...
class _VoucherListScreenState extends State<VoucherListScreen> {
      ...
      builder: (BuildContext context) {
        ...
        return AlertDialog(
          ...
          content: Column(
            mainAxisSize: MainAxisSize.min,
            children: const [
              Icon(Icons.check_circle, color: Colors.blue, size: 50), // Icon xanh dương
              SizedBox(height: 16),
              Text('Lưu voucher thành công!', style: TextStyle(fontSize: 16, color: Colors.blue)),
            ],
          ),
        );
      },
  ...
  Future<void> _saveVoucher(VoucherModel voucher) async {
    ...
    try {
      ...
      if (confirmSave == true && user.memberShipCurrentPoint! > voucher.pointNeeded) {
        await _userVoucherRepo.addUserVoucher(userVoucher);
        await _userRepo.subtractMembershipCurrentPoints(
          _userModel!.id!,
          voucher.pointNeeded,
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(e.toString(), style: TextStyle(color: Colors.blue.shade700)),
          backgroundColor: Colors.white,
        ),
      );
    }
                ...
                children: [
                  Text(
                    ...
                    style: const TextStyle(
                      fontSize: 16,
                      fontWeight: FontWeight.bold,
                      color: Colors.blue, // Văn bản xanh dương
                    ),
                  ),
                  const SizedBox(height: 4),
                  Text(
                    "Giảm ${Utils.formatCurrency(voucher.discountAmount)}",
                    style: const TextStyle(
                      color: Colors.blue, // Thay màu đỏ bằng xanh dương
                      fontWeight: FontWeight.w500,
                    ),
                  ),
                  const SizedBox(height: 4),
                  Text(
                    "Đã sử dụng: ${voucher.currentUsage}/${voucher.maxUsage}",
                    style: TextStyle(
                      color: voucher.isValid ? Colors.green : Colors.blue.shade700, // Xanh lá hoặc xanh đậm
                      fontSize: 12,
                    ),
                  ),
                  const SizedBox(height: 4),
                  Text(
                    "Điểm cần để đổi: ${voucher.pointNeeded} điểm",
                    style: const TextStyle(color: Colors.blue, fontSize: 12), // Xanh dương
                  ),
                ],
              ),
            ),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: _vouchers.isEmpty
          ? const Center(child: Text('Không có voucher nào', style: TextStyle(color: Colors.blue)))
          : ListView.separated(
              padding: const EdgeInsets.all(16),
              itemCount: _vouchers.length,
              separatorBuilder: (context, index) => const SizedBox(height: 16),
              itemBuilder: (context, index) => _buildVoucherItem(_vouchers[index]),
            ),
    );
  }
}